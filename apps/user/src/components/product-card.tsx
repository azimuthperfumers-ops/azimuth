...
type Product = {
  ...
  variants: { sellingPrice: string; mrp: string; status: string }[];
};
  ...
  return (
    <Link href={`/shop/${product.slug ?? product.id}`} className="group block">
      ...
      <div className="pt-3.5 pb-1">
        <h3 className="font-heading text-[1.05rem] font-medium leading-snug tracking-tight text-foreground">
          {product.name}
        </h3>
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          {fromPrice !== null && (
            <div className="flex items-baseline gap-1.5 shrink-0">
              ...
              {hasDiscount && (
                <span className="text-[10px] tabular-nums text-muted-foreground/45 line-through">
